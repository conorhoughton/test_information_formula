...
\begin{frame}{Differential entropy is tricky}
  \color{dark}
$$
H(X)=-\int dx p(x)\log_2 {p(x)} \approx -\sum_i \delta x p(x_i) \log_2{p(x_i)}
$$
\color{black}so if $p_i$\color{black}{} is the probability for the $i$\color{black}th bin then
\color{dark}
$$
H(X)\approx - \sum_i p_i \log{p_i} +\log{\delta x}
$$
