...
\color{black}
\vskip 1cm
If $S$ is the stimulus $H(X|S)$ quantifies noise.
\end{frame}
...
\color{black}
where $\textbf{x}_0$ is a point, for example, in the middle of $B$ the region we are interested in. Now
\color{dark}
...
\end{center}
\color{black}
In this case $B$ is the ball and
\color{dark}
$$p(\circ)=\frac{4}{1053\mbox{vol}\,B}$$
\color{black}
since $n=1053$ is the total number of holes in the board.
\end{frame}
...
\end{center}
\color{black}
A ball with volume $h/n$ around the circled point, where $n$ is the total number of holes. Here $h=4$ so the volume is $4/1053$.
\end{frame}
